...
public class ColorPickerView extends View {
		...
		public interface OnColorChangedListener {
			public void onColorChanged(final int pAlpha, final float pHue, final float pSat, final float pVal);
		
		}

		public ColorPickerView(Context pContext) {
			this(pContext, null);
		}

		public ColorPickerView(Context pContext, AttributeSet pAttrs) {
			this(pContext, pAttrs, 0);
		}

		public ColorPickerView(Context pContext, AttributeSet pAttrs, int pDefStyle) {
		...
		protected void onDraw(Canvas pCanvas) {
			...
			drawSatValPanel(pCanvas);
			drawHuePanel(pCanvas);
			drawAlphaPanel(pCanvas);
		}

		private void drawSatValPanel(Canvas pCanvas) {

			final RectF rect = mSatValRect;

			if (BORDER_WIDTH_PX > 0) {
				mBorderPaint.setColor(mBorderColor);
				pCanvas.drawRect(mDrawingRect.left, mDrawingRect.top, rect.right
			...
			pCanvas.drawRect(rect, mSatValPaint);

			final Point p = satValToPoint(mSat, mVal);

			mSatValTrackerPaint.setColor(0xff000000);
			pCanvas.drawCircle(p.x, p.y, PALETTE_CIRCLE_TRACKER_RADIUS - 1f
					* mDensity, mSatValTrackerPaint);

			mSatValTrackerPaint.setColor(0xffdddddd);
			pCanvas.drawCircle(p.x, p.y, PALETTE_CIRCLE_TRACKER_RADIUS,
					mSatValTrackerPaint);

		}

		private void drawHuePanel(Canvas pCanvas) {

			final RectF rect = mHueRect;

			if (BORDER_WIDTH_PX > 0) {
				mBorderPaint.setColor(mBorderColor);
				pCanvas.drawRect(rect.left - BORDER_WIDTH_PX, rect.top
			...
			pCanvas.drawRect(rect, mHuePaint);
			...
			pCanvas.drawRoundRect(r, 2, 2, mHueTrackerPaint);

		}

		private void drawAlphaPanel(Canvas pCanvas) {
			...
			pCanvas.drawRect(rect, mAlphaPaint);

			if (mAlphaSliderText != null && mAlphaSliderText != "") {
				pCanvas.drawText(mAlphaSliderText, rect.centerX(), rect.centerY()
		...
		private Point hueToPoint(float pHue) {
			...
			p.y = (int) (height - (pHue * height / 360f) + rect.top);
			p.x = (int) rect.left;

			return p;
		}

		private Point satValToPoint(float pSat, float pVal) {
			...
			p.x = (int) (pSat * width + rect.left);
			p.y = (int) ((1f - pVal) * height + rect.top);

			return p;
		}

		private Point alphaToPoint(int pAlpha) {
			...
			p.x = (int) (width - (pAlpha * width / 0xff) + rect.left);
			p.y = (int) rect.top;
		...
		private float[] pointToSatVal(float pX, float pY) {
			...
			result[0] = 1.f / width * pX;
			result[1] = 1.f - (1.f / height * pY);

			return result;
		}

		private float pointToHue(float pY) {
			...
			return 360f - (pY * 360f / height);
		}

		private int pointToAlpha(int pX) {
		...
		public boolean onTrackballEvent(MotionEvent pEvent) {
			...
			if (update) {
				...
				if (mListener != null) {
					mListener.onColorChanged(mAlpha, mHue, mSat, mVal);
				}
			...
			return super.onTrackballEvent(pEvent);
		}
		...
		public boolean onTouchEvent(MotionEvent pEvent) {
			...
			case MotionEvent.ACTION_DOWN:
				mStartTouchPoint = new Point((int) pEvent.getX(), (int) pEvent.getY());
				update = moveTrackersIfNeeded(pEvent);
				break;
		...
		private boolean moveTrackersIfNeeded(MotionEvent pEvent) {
			...
			if (mHueRect.contains(startX, startY)) {
				...
				mHue = pointToHue(pEvent.getY());

				update = true;
			} else if (mSatValRect.contains(startX, startY)) {
				mLastTouchedPanel = PANEL_SAT_VAL;
				float[] result = pointToSatVal(pEvent.getX(), pEvent.getY());
				mSat = result[0];
				mVal = result[1];
				update = true;
			} else if (mAlphaRect != null && mAlphaRect.contains(startX, startY)) {
				mLastTouchedPanel = PANEL_ALPHA;
				mAlpha = pointToAlpha((int) pEvent.getX());
				update = true;
			}
			return update;
		}
		...
		protected void onMeasure(int pWidthMeasureSpec, int pHeightMeasureSpec) {
			...
			int widthAllowed = MeasureSpec.getSize(pWidthMeasureSpec);
			int heightAllowed = MeasureSpec.getSize(pHeightMeasureSpec);
		...
		private int chooseWidth(int pMode, int pSize) {
			if (pMode == MeasureSpec.AT_MOST || pMode == MeasureSpec.EXACTLY) {
				return pSize;
		...
		private int chooseHeight(int pMode, int pSize) {
			if (pMode == MeasureSpec.AT_MOST || pMode == MeasureSpec.EXACTLY) {
				return pSize;
			} else { // (mode == MeasureSpec.UNSPECIFIED)
		...
		protected void onSizeChanged(int pW, int pH, int pOldw, int pOldh) {
			...
			mDrawingRect.right = pW - mDrawingOffset - getPaddingRight();
			mDrawingRect.top = mDrawingOffset + getPaddingTop();
			mDrawingRect.bottom = pH - mDrawingOffset - getPaddingBottom();
		 ...
		 * @param listener
		 */
		public void setOnColorChangedListener(OnColorChangedListener pListener) {
			mListener = pListener;
		}
		 ...
		 */
		public void setColor(int pColor, boolean pCallback) {
		 ...
		 * @param visible
		 */
		public void setAlphaSliderVisible(boolean pVisible) {

			if (mShowAlphaPanel != pVisible) {
				mShowAlphaPanel = pVisible;
		...
		public void setSliderTrackerColor(int pColor) {
			mSliderTrackerColor = pColor;
		 ...
		 *            string resource id.
		 */
		public void setAlphaSliderText(int pRes) {
			String text = getContext().getString(pRes);
		 ...
		 */
		public void setAlphaSliderText(String pText) {
			mAlphaSliderText = pText;
			invalidate();
		}
